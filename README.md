...
## 今後の予定
現在はシミュレータでのみ実装していて、実機ではまだ実装しきれていない。また、シミュレータでは完全なばら積み状態では精度が明らかに悪い状況である。今後は一つ一つの処理のロバスト性を高めながら、実機でも適用できた動画を載せられるようにあと4ヶ月ほどの共同研究の時間を取り組んでいきたい。
